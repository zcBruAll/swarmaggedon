...
export const FIELD_REPAIR_RATE = 12;
const ORBIT_REPAIR_RATE = 6;
...
const DRONE_IFRAME_DURATION = 0.35;

export function createDrone(index, weaponType, enchant = undefined) {
    const weapon = createWeapon(weaponType, enchant);
    weapon.damage = parseFloat((weapon.damage * 0.88).toFixed(2));
    weapon.cooldown = parseFloat((weapon.cooldown * 1.05).toFixed(2));
    ...
    return {
        ...
        _updateOrbiting(dt, world, player) {
            const orbitingDrones = world.actors
                .filter(a => a.drawType === 'drone' && a.state === DRONE_STATE.ORBITING)
                .sort((a, b) => a.index - b.index);

            const total = orbitingDrones.length || 1;
            const slot = orbitingDrones.indexOf(this);

            const slotOffset = (slot / total) * Math.PI * 2;
            const a = orbitPhase.value + slotOffset;
        ...
        _updateDeployed(dt, world, player) {
            if (this._deployTimer > 0) { this._deployTimer -= dt; return; }
        ...
        resetForNextWave(player) {
            ...
            this.weapon.cooldownTime = 0;
            switch (this.state) {
                case DRONE_STATE.DEPLOYED: this.state = DRONE_STATE.RECALLING; break;
